...
def get_tribunals_court_cases_data(input_data):
    try:
        ...
        if starting_page_response.status_code != 200:
            logger.info('we have not got starting page response successfully')

        soup = BeautifulSoup(starting_page_response.content, 'html.parser')
        if soup.select('form#FormId>input') != []:
            ...
            if court_cases_data_list != None:
                ...
                return [{"message": "Server error, Please try after some time!"}]
        else:
            return [{"message": "Server error, Please try after some time!"}]
    except:
...
def get_tribunals_token_values(input_data):
    try:
        starting_page_headers = {
            'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
            'Accept-Encoding': 'gzip, deflate, br',
        ...
        if starting_page_response.status_code != 200:
            logger.info('we have not got proper starting page response')

        soup = BeautifulSoup(starting_page_response.content, 'html.parser')
        token_value = soup.select('form#FormId>input')[0].get('value')

        token_values_payload = f"_token={token_value}&search_by=filing_no_wise&location={input_data.get('location')}&case_type=&case_status=&select_party=&party_name=&diary_no={input_data.get('filing_no')}&case_number=&advocate_name=&case_year="
        
        token_values_headers = {
            'Accept': '*/*',
            'Accept-Encoding': 'gzip, deflate, br',
            'Accept-Language': 'en-US,en;q=0.9,hi;q=0.8',
            'Connection': 'keep-alive',
            'Content-Length': f'{len(token_values_payload)}',
            'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8',
            'Cookie': f'_ga=GA1.3.1468851884.1678795714; _gid=GA1.3.347674307.1678795714; XSRF-TOKEN={get_cookies_data()["XSRF-TOKEN"]}; laravel_session={get_cookies_data()["laravel_session"]}',
            'Host': 'nclat.nic.in',
            'Origin': 'https://nclat.nic.in',
        ...
        token_values_response = session.post("https://nclat.nic.in/display-board/cases_details", headers=token_values_headers, data=token_values_payload, verify=False)
        
        if token_values_response.status_code != 200:
            logger.info('we have not got token values response successfully')

        cookies_value = session.cookies.get_dict()
        xsrf_token = cookies_value.get('XSRF-TOKEN')
        laravel_session = cookies_value.get('laravel_session')

        return token_value, xsrf_token, laravel_session

    except:
        logger.info(f"Error in get_tribunals_token_values :- {traceback.format_exc()}")


# print(get_tribunals_court_cases_data(input_data))
# print(get_tribunals_token_values(input_data))
